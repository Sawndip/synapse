#ifndef DTRIANGULAR_HH
#define DTRIANGULAR_HH
...
/** @brief Defines the methods associated with the Triangular distribution in any dimension
 */
class DTriangular : public DFunction {
 public:

  /** @brief Basic constructor, initilizes an triangular distribution of dimension n
   *
   *  @param 	n	Dimension of the space
   */
  DTriangular(const size_t n=1);

  /** @brief 1D constructor, initilizes an triangular distribution of mean mu and scale lambda
   *
   *  @param 	mean	Mean of the distribution
   *  @param 	lambda	Scale of the distribution
   */
  DTriangular(const double mean,
	      const double lambda);

  /** @brief nD constructor, initilizes an triangular distibution of given means and scales
   *
   *  @param 	mean	Mean of the distribution
   *  @param 	lambda	Scales of the distribution
   */
  DTriangular(const std::vector<double>& mean,
	      const std::vector<double>& lambda);

  /** @brief Copy constructor */
  DTriangular(const DTriangular& df);

  /** @brief Equality operator */
  DTriangular& operator=(const DTriangular& df);
