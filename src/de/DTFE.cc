...
DTFE::DTFE(const std::vector<Vertex>& vertices) :
...
DTFE::DTFE(const std::vector<std::vector<double>>& vertices) :
...
DTFE::DTFE(const DTFE& del) {
  *this = del;
}

DTFE& DTFE::operator=(const DTFE& del) {
  if ( this == &del )
      return *this;
...
bool DTFE::Initialize() {

  // Check that there are vertices to fit and there are at least n+1 vertices for dimension n
  Assert::IsNotEmpty("DTFE::Initialize", "Vector of input points", _vertices);
  Assert::IsNonZero("DTFE::Initialize", "Dimension", _vertices[0].size());
  ...
  // Feed the vector to Qhull and compute the DTFE triangulation ("d")
  ...
  _qhull->defineVertexNeighborFacets();

  // Compute the density value at each of the vertices by taking the average simplex
  // volume of the neighbouring simplices
  double meanvol;
  for (const orgQhull::QhullVertex& vertex : _qhull->vertexList().toStdVector()) {
    meanvol = 0.;
    for (orgQhull::QhullFacet& facet : vertex.neighborFacets().toStdVector())
        meanvol += facet.facetArea();

//    meanvol /= vertex.neighborFacets().toStdVector().size();

    _vertices[vertex.point().id()].SetValue(1./meanvol/N/2);
  }

  // Store the facets for fast access. Each facet is an array of n+1 n-points (simplex)
  // Add the mapping of the vertex as the density value at that point
...
const std::vector<Vertex>& DTFE::GetBestFacet(const std::vector<double>& v, bool* isin) const {
  ...
  // QHull only knows if the point is in the circumcircle of the DTFE facet,
  // should check manually to ensure containment, if required (TODO)
  *isin = in;
/*  *isin = false;
...
std::vector<TPolyLine*> DTFE::Polygons() const {

  // Throw if the wrong dimension is requested
  std::vector<TPolyLine*> polylines;
  Assert::IsEqual("DTFE::Polygons", "Dimension", _dim, (size_t)2);
