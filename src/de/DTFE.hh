...
/** @brief Computes the Delaunay Tesselation Field Estimator (DTFE) of a set of points
 *
 *   	   The Delaunary triangulation is obtained by computing the (n+1)-dimensional
 ...
 *
 * 	   The DTFE uses the average neighbouring simplex volume as a driver for density estimation
 */
class DTFE {
 public:
   ...
   */
  DTFE(const std::vector<Vertex>& vertices);

  /** @brief Normal constructor, sets the vector of points
   *
   *  @param	vertices	Vector of points
   */
  DTFE(const std::vector<std::vector<double>>& vertices);

  /** @brief Copy constructor */
  DTFE(const DTFE& del);

  /** @brief Equality operator */
  DTFE& operator=(const DTFE& del);

  /** @brief Destructor */
  ~DTFE();

  /** @brief Find the DTFE triangulation of the N vertices */
  bool Initialize();
